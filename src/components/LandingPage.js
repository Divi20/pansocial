      ...
      <header className="App-header">
        <div className="accountscontainer">
          <Card className="accountscard">
            ...
            <CardBody className="mainListContainer">
                
                <ul className="list-groupmb-5">
                  {users.map(user => (
                    <Link to="/home">
                      <li key={user.id} className="list-group-item" onClick={() => setCurrentUser([user])}>
                        <img className="mainlistimage" src={user.profilepicture} alt="pp"/> {user.name}
                      </li>
                    </Link>
                  ))}
                </ul>
